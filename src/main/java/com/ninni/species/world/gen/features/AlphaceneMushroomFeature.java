...
public class AlphaceneMushroomFeature extends Feature<NoneFeatureConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> featurePlaceContext) {
        ...
        BlockState mushroomBlock = SpeciesBlocks.ALPHACENE_MUSHROOM_BLOCK.get().defaultBlockState();
    ...
    private void placeConks(WorldGenLevel worldGenLevel, RandomSource randomSource, List<BlockPos> conkPoses) {
        for (BlockPos pos : conkPoses) {
            if (worldGenLevel.isEmptyBlock(pos) && worldGenLevel.getBlockState(pos.above()).is(SpeciesBlocks.ALPHACENE_MUSHROOM_BLOCK.get()) && randomSource.nextInt(10) != 0) {
                worldGenLevel.setBlock(pos, SpeciesBlocks.ALPHACENE_MUSHROOM_GROWTH.get().defaultBlockState(), 2);
            }
        }
    }
    ...
    private void placeStem(int primaryStemLength, int secondLayerCount, WorldGenLevel world, BlockPos blockPos, BlockState stemBlock, BlockState mushroomBlock) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            boolean placeable = world.isStateAtPosition(blockPos.above(i), DripstoneUtils::isEmptyOrWater) || world.getBlockState(blockPos).is(SpeciesBlocks.ALPHACENE_MUSHROOM.get());
