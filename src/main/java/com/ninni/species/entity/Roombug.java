...
public class Roombug extends TamableAnimal {
    ...
    public InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (!this.isTame() && TAMING_INGREDIENTS.contains(itemStack.getItem())) {
            ...
            if (!this.isSilent()) {
                this.level().playSound(null, this.getX(), this.getY(), this.getZ(), SpeciesSoundEvents.ROOMBUG_EAT.get(), this.getSoundSource(), 1.0f, 1.0f + (this.random.nextFloat() - this.random.nextFloat()) * 0.2f);
            }
    ...
    protected SoundEvent getAmbientSound() {
        return this.isOrderedToSit() ? SpeciesSoundEvents.ROOMBUG_SNORING.get() : SpeciesSoundEvents.ROOMBUG_IDLE.get();
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource source) {
        return SpeciesSoundEvents.ROOMBUG_HURT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return SpeciesSoundEvents.ROOMBUG_DEATH.get();
    }
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        if (this.getName().getString().equalsIgnoreCase("goofy aah") || this.getName().getString().equalsIgnoreCase("goofy ah") || this.getName().getString().equalsIgnoreCase("goofy ahh")) {
            this.playSound(SpeciesSoundEvents.ROOMBUG_GOOFY_AAH_STEP.get(), 1, 1);
        } else this.playSound(SpeciesSoundEvents.ROOMBUG_STEP.get(), 0.5f, 1.0f);
    }
