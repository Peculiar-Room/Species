...
public class SpitPellet extends Behavior<Cruncher> {
    ...
    public SpitPellet() {
        super(ImmutableMap.of(
                ...
                SpeciesMemoryModuleTypes.SPIT_CHARGING.get(), MemoryStatus.VALUE_ABSENT
        ), DURATION);
    }
    ...
    protected void start(ServerLevel serverLevel, Cruncher livingEntity, long l) {
        ...
        livingEntity.playSound(SpeciesSoundEvents.CRUNCHER_SPIT.get(), 2.0F, 1.0F);
        livingEntity.getBrain().setMemoryWithExpiry(SpeciesMemoryModuleTypes.SPIT_CHARGING.get(), Unit.INSTANCE, 12);
    }
    ...
    protected void tick(ServerLevel serverLevel, Cruncher livingEntity, long l) {
        ...
        if (brain.getMemory(SpeciesMemoryModuleTypes.SPIT_CHARGING.get()).isPresent()) return;

        BlockPos blockPos = livingEntity.blockPosition();

        BlockState blockState = SpeciesBlocks.CRUNCHER_PELLET.get().defaultBlockState();
